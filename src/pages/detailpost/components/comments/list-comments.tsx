...
export function ListComments() {
    ...
    const handleCalcComments = (): number => {
        ...
        if (currentComments + increment > maxLengthComments) {
            setCurrentComments(
                currentComments + (maxLengthComments - currentComments),
            );
        } else {
    ...
    return (
        <div className={styles.container}>
            ...
            {comments.length != currentComments ? (
                <Text
                    className={styles.seemorecomments}
                    strong
                    onClick={handleCalcComments}
                >
                    See more comments
                </Text>
            ) : (
...
const Comment: React.FC<{ data: IComment }> = ({ data }) => {
    ...
    return (
        <div
            ...
            style={{ marginLeft: `${35 * Number(data.level)}px` }}
        >
                ...
                <Col flex={"auto"} className={styles.right}>
                    <Row justify={"start"} align={"top"}>
                        ...
                        <Flex gap={"small"} justify={"flex-start"}>
                            <Text
                                className={styles.acincmt}
                                strong
                                onClick={handleChangeStateReply}
                            >
                                Reply
                            </Text>
                            {data?.comments_replies && (
                                <Text
                                    className={styles.acincmt}
                                    strong
                                    onClick={handleChangeStateHidden}
                                >
                                    {!isHidden ? "More" : "Less"}
                                </Text>
                            )}
            ...
            {data?.comments_replies && isHidden && (
                <ListCommentBase data={data?.comments_replies} />
            )}
...
const ListCommentBase: React.FC<{ data?: IComment[] }> = ({ data }) => {
    return (
        <>
